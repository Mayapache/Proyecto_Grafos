...
class Grafo:
    ...
    #[label="hello world"]            
        ...
        dot_str+="}"
        with open('C:/Users/Charlie/Documents/Maestria/1er_Semestre/Diseño y análisis de algoritmos/Proyecto/Parte_3/'+nombre_doc+'.gv', 'w') as f:
            f.write(dot_str)
        print(dot_str)
...
def Dijikstra(a):
    ...
    destino=round(random.random()*(Nu_vert-1))
    ...
    lis_nod_rec=[origen]
    lis_nod_rec_pes=[0]
    lis_ari_rec=[] #Para hacer el grafo, resultado    
    lis_nod_ady=np.asarray(np.where(mat_vert[origen]>0)[0])
    lis_ari_ady=[]
    for i in range(len(lis_nod_ady)):
        lis_ari_ady.append([origen,lis_nod_ady[i]])
    lis_pes_ari_ady=mat_vert[origen][lis_nod_ady]
    lis_pes_nod_ady=mat_vert[origen][lis_nod_ady]    
    ...
    while len(lis_nod_ady): #Mientras hayan nodos por explorar
        selec=np.where(lis_pes_nod_ady==min(lis_pes_nod_ady))[0] #seleccionamos el de menor peso
        selec=selec[0]
        #Actualizamos los recorridos
